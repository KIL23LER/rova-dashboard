"""
api_server.py — Rova Bot v5.0 ULTRA
...
async def _discord(method: str, path: str, token: str, bot: bool = False):
    auth = f"Bot {token}" if bot else f"Bearer {token}"
    async with aiohttp.ClientSession() as s:
        async with s.request(method, f"{DISCORD_API}{path}",
                             headers={"Authorization": auth}) as r:
            if not r.ok:
                return [] if r.status == 404 else None
            return await r.json()

def _patch(db, table: str, g: str, body: dict, col_map: dict, json_cols: set = None):
    sets, vals = [], []
    for key, col in col_map.items():
        ...
        vals.append(v)
    if sets:
        db.execute(f"UPDATE {table} SET {','.join(sets)} WHERE guild_id=?", (*vals, g))
...
async def guilds_list(request: web.Request) -> web.Response:
    ...
    if not sess:
        return json_resp({"error": "Not authenticated"}, 401)
    try:
        ug = await _discord("GET", "/users/@me/guilds?with_counts=true", sess["access_token"]) or []
        bg = await _discord("GET", "/users/@me/guilds", BOT_TOKEN, bot=True) or []
        bot_ids = {g["id"] for g in bg if isinstance(g, dict)}
        def has_manage(p):
            try: return bool(int(p) & 0x20)
            except: return False
        result = [
            {"id": g["id"], "name": g["name"], "icon": g.get("icon"),
             "botPresent": g["id"] in bot_ids,
             "memberCount": g.get("approximate_member_count", 0),
             "owner": g.get("owner", False)}
            for g in ug
            if isinstance(g, dict) and (g.get("owner") or has_manage(str(g.get("permissions", 0))))
        ]
...
async def guild_detail(request: web.Request) -> web.Response:
    ...
    ch = [{"id": c["id"], "name": c["name"], "type": c["type"]}
          for c in channels if isinstance(c, dict) and c.get("type") in (0, 2, 4, 5, 15)]
    ro = sorted(
        [{"id": r["id"], "name": r["name"], "color": r.get("color", 0)}
         for r in roles if isinstance(r, dict) and r.get("name") != "@everyone"],
...
async def settings_patch(request: web.Request) -> web.Response:
    ...
    with get_db() as db:
        ...
        if "prefix" in body:
            db.execute("UPDATE guild_config SET prefix=? WHERE guild_id=?", (body["prefix"], g))
...
async def welcome_get(request: web.Request) -> web.Response:
    ...
    return json_resp({"guildId": g, "enabled": bool(row.get("enabled")), "channelId": row.get("channel_id"),
                      "message": row.get("message"), "embedColor": row.get("embed_color", "#a855f7")})
...
async def welcome_patch(request: web.Request) -> web.Response:
    ...
    return json_resp({"guildId": g, "enabled": bool(row.get("enabled")), "channelId": row.get("channel_id"),
                      "message": row.get("message"), "embedColor": row.get("embed_color")})
...
async def leave_get(request: web.Request) -> web.Response:
    ...
    return json_resp({"guildId": g, "enabled": bool(row.get("enabled")), "channelId": row.get("channel_id"),
                      "message": row.get("message"), "embedColor": row.get("embed_color", "#ef4444")})
...
async def protection_get(request: web.Request) -> web.Response:
    ...
    return json_resp({
        "guildId": g,
        "antispamEnabled": bool(row.get("antispam_enabled")),
        "antispamMessages": row.get("antispam_messages", 5),
        "antispamSeconds": row.get("antispam_seconds", 4),
        "antispamAction": row.get("antispam_action", "timeout"),
        "antilinkEnabled": bool(row.get("antilink_enabled")),
        "antilinkWhitelist": json.loads(row.get("antilink_whitelist") or "[]"),
        "antiraidEnabled": bool(row.get("antiraid_enabled")),
        "antiraidJoins": row.get("antiraid_joins", 8),
        "antiraidSeconds": row.get("antiraid_seconds", 10),
        "antiraidAction": row.get("antiraid_action", "kick"),
        "antimentionsEnabled": bool(row.get("antimentions_enabled")),
        "antimentionsLimit": row.get("antimentions_limit", 5),
        "badwordsEnabled": bool(row.get("badwords_enabled")),
        "badwords": json.loads(row.get("badwords") or "[]"),
    })
...
async def protection_patch(request: web.Request) -> web.Response:
    ...
    return json_resp({
        "guildId": g,
        "antispamEnabled": bool(row.get("antispam_enabled")),
        "antispamMessages": row.get("antispam_messages"),
        "antispamSeconds": row.get("antispam_seconds"),
        "antispamAction": row.get("antispam_action"),
        "antilinkEnabled": bool(row.get("antilink_enabled")),
        "antilinkWhitelist": json.loads(row.get("antilink_whitelist") or "[]"),
        "antiraidEnabled": bool(row.get("antiraid_enabled")),
        "antiraidJoins": row.get("antiraid_joins"),
        "antiraidSeconds": row.get("antiraid_seconds"),
        "antiraidAction": row.get("antiraid_action"),
        "antimentionsEnabled": bool(row.get("antimentions_enabled")),
        "antimentionsLimit": row.get("antimentions_limit"),
        "badwordsEnabled": bool(row.get("badwords_enabled")),
        "badwords": json.loads(row.get("badwords") or "[]"),
    })
...
async def antinuke_get(request: web.Request) -> web.Response:
    ...
    return json_resp({
        "guildId": g, "enabled": bool(row.get("enabled")),
        "banThreshold": row.get("ban_threshold", 3), "kickThreshold": row.get("kick_threshold", 3),
        "channelThreshold": row.get("channel_threshold", 3), "roleThreshold": row.get("role_threshold", 3),
        "webhookThreshold": row.get("webhook_threshold", 3), "punishment": row.get("punishment", "ban"),
        "whitelist": json.loads(row.get("whitelist") or "[]"),
    })
...
async def antinuke_patch(request: web.Request) -> web.Response:
    ...
    with get_db() as db:
        ...
        _patch(db, "antinuke_config", g, body, {
    ...
    return json_resp({
        "guildId": g, "enabled": bool(row.get("enabled")),
        "banThreshold": row.get("ban_threshold"), "kickThreshold": row.get("kick_threshold"),
        "channelThreshold": row.get("channel_threshold"), "roleThreshold": row.get("role_threshold"),
        "webhookThreshold": row.get("webhook_threshold"), "punishment": row.get("punishment"),
        "whitelist": json.loads(row.get("whitelist") or "[]"),
    })
...
async def leveling_get(request: web.Request) -> web.Response:
    ...
    return json_resp({"guildId": g, "enabled": bool(row.get("enabled")), "xpMin": row.get("xp_min", 15),
                      "xpMax": row.get("xp_max", 40), "cooldownSeconds": row.get("cooldown_seconds", 60),
                      "levelupChannel": row.get("levelup_channel"), "levelupMessage": row.get("levelup_message")})
...
async def leveling_patch(request: web.Request) -> web.Response:
    ...
    with get_db() as db:
        ...
        _patch(db, "leveling_config", g, body, {"enabled":"enabled","xpMin":"xp_min","xpMax":"xp_max",
    ...
    return json_resp({"guildId": g, "enabled": bool(row.get("enabled")), "xpMin": row.get("xp_min"),
                      "xpMax": row.get("xp_max"), "cooldownSeconds": row.get("cooldown_seconds"),
                      "levelupChannel": row.get("levelup_channel"), "levelupMessage": row.get("levelup_message")})
...
async def tickets_get(request: web.Request) -> web.Response:
    ...
    return json_resp({"guildId": g, "panelChannel": row.get("panel_channel"), "logChannel": row.get("log_channel"),
                      "supportRole": row.get("support_role"), "categoryId": row.get("category_id"),
                      "ticketCount": row.get("ticket_count", 0)})
...
async def tickets_patch(request: web.Request) -> web.Response:
    ...
    with get_db() as db:
        ...
        _patch(db, "ticket_config", g, body, {"panelChannel":"panel_channel","logChannel":"log_channel",
            "supportRole":"support_role","categoryId":"category_id"})
...
async def suggestions_get(request: web.Request) -> web.Response:
    ...
    return json_resp({"guildId": g, "enabled": bool(row.get("enabled")), "channelId": row.get("channel_id"),
                      "logChannel": row.get("log_channel")})
...
async def autoroles_post(request: web.Request) -> web.Response:
    ...
    with get_db() as db:
        db.execute("INSERT OR IGNORE INTO autoroles (guild_id, role_id, bot_only) VALUES (?,?,?)",
                   (g, rid, 1 if body.get("botOnly") else 0))
...
async def giveaways_get(request: web.Request) -> web.Response:
    ...
    return json_resp([{
        "id": r["id"], "guildId": r["guild_id"], "channelId": r["channel_id"],
        "hostId": r["host_id"], "prize": r["prize"], "winners": r["winners"],
        "endsAt": r["ends_at"], "ended": bool(r["ended"]),
        "entryCount": len(json.loads(r["entries"] or "[]")),
    } for r in rows])
...
async def economy_get(request: web.Request) -> web.Response:
    ...
    with get_db() as db:
        rows = db.execute(
            "SELECT user_id, wallet, bank, wallet+bank as total FROM economy WHERE guild_id=? ORDER BY total DESC LIMIT 20", (g,)
        ).fetchall()
    return json_resp([{"userId": r["user_id"], "wallet": r["wallet"], "bank": r["bank"], "total": r["total"]} for r in rows])

# ─── Reaction Roles ───────────────────────────────────────────────────────────

async def reactionroles_get(request: web.Request) -> web.Response:
    if not require_auth(request): return json_resp({"error": "Not authenticated"}, 401)
    g = gid(request)
    with get_db() as db:
        rows = db.execute("SELECT * FROM reaction_roles WHERE guild_id=?", (g,)).fetchall()
    return json_resp([{"guildId": r["guild_id"], "messageId": r["message_id"], "channelId": r["channel_id"],
                       "emoji": r["emoji"], "roleId": r["role_id"]} for r in rows])

async def reactionroles_delete(request: web.Request) -> web.Response:
    if not require_auth(request): return json_resp({"error": "Not authenticated"}, 401)
    g = gid(request); body = await request.json()
    with get_db() as db:
        db.execute("DELETE FROM reaction_roles WHERE guild_id=? AND message_id=? AND emoji=?",
                   (g, body.get("messageId"), body.get("emoji")))
    return json_resp({"ok": True})

# ─── Birthday ─────────────────────────────────────────────────────────────────

async def birthday_get(request: web.Request) -> web.Response:
    if not require_auth(request): return json_resp({"error": "Not authenticated"}, 401)
    g = gid(request)
    with get_db() as db:
        cfg      = _ensure(db, "birthday_config", g)
        bdays    = db.execute("SELECT * FROM birthdays WHERE guild_id=? ORDER BY month, day", (g,)).fetchall()
    return json_resp({
        "guildId": g, "enabled": bool(cfg.get("enabled")), "channelId": cfg.get("channel_id"),
        "message": cfg.get("message", "عيد ميلاد سعيد {user}! 🎂"),
        "birthdays": [{"userId": r["user_id"], "month": r["month"], "day": r["day"]} for r in bdays],
    })

async def birthday_patch(request: web.Request) -> web.Response:
    if not require_auth(request): return json_resp({"error": "Not authenticated"}, 401)
    g = gid(request); body = await request.json()
    with get_db() as db:
        db.execute("INSERT OR IGNORE INTO birthday_config (guild_id) VALUES (?)", (g,))
        _patch(db, "birthday_config", g, body, {"enabled":"enabled","channelId":"channel_id","message":"message"})
        cfg = dict(db.execute("SELECT * FROM birthday_config WHERE guild_id=?", (g,)).fetchone())
    return json_resp({"guildId": g, "enabled": bool(cfg.get("enabled")), "channelId": cfg.get("channel_id"),
                      "message": cfg.get("message")})

# ─── Podcast ──────────────────────────────────────────────────────────────────

async def podcast_get(request: web.Request) -> web.Response:
    if not require_auth(request): return json_resp({"error": "Not authenticated"}, 401)
    g = gid(request)
    with get_db() as db:
        cfg      = _ensure(db, "podcast_config", g)
        episodes = db.execute("SELECT * FROM podcast_episodes WHERE guild_id=? ORDER BY created_at DESC LIMIT 10", (g,)).fetchall()
    return json_resp({
        "guildId": g, "enabled": bool(cfg.get("enabled")), "roleId": cfg.get("role_id"),
        "episodes": [{"id": e["id"], "title": e["title"], "content": e["content"],
                      "sentAt": e["sent_at"], "createdAt": e["created_at"]} for e in episodes],
    })

async def podcast_patch(request: web.Request) -> web.Response:
    if not require_auth(request): return json_resp({"error": "Not authenticated"}, 401)
    g = gid(request); body = await request.json()
    with get_db() as db:
        db.execute("INSERT OR IGNORE INTO podcast_config (guild_id) VALUES (?)", (g,))
        _patch(db, "podcast_config", g, body, {"enabled":"enabled","roleId":"role_id"})
        cfg = dict(db.execute("SELECT * FROM podcast_config WHERE guild_id=?", (g,)).fetchone())
    return json_resp({"guildId": g, "enabled": bool(cfg.get("enabled")), "roleId": cfg.get("role_id")})

# ─── Announcements ────────────────────────────────────────────────────────────

async def announcements_get(request: web.Request) -> web.Response:
    if not require_auth(request): return json_resp({"error": "Not authenticated"}, 401)
    g = gid(request)
    with get_db() as db:
        rows = db.execute("SELECT * FROM announcements WHERE guild_id=? ORDER BY sent_at DESC LIMIT 20", (g,)).fetchall()
    return json_resp([{"id": r["id"], "guildId": r["guild_id"], "channelId": r["channel_id"],
                       "title": r["title"], "content": r["content"], "color": r["color"],
                       "sentAt": r["sent_at"]} for r in rows])
...
async def bot_stats_get(_request: web.Request) -> web.Response:
    ...
    return json_resp({"guildCount": row["guild_count"], "memberCount": row["member_count"],
                      "commandCount": row["command_count"], "uptime": row["uptime"]})

async def health(_request: web.Request) -> web.Response:
    return json_resp({"ok": True, "service": "Rova Bot v5.0 API", "time": int(time.time())})
...
def create_app() -> web.Application:
    ...
    routes = [
        ...
        # ── Economy ──
        ("GET",    "/api/guilds/{guildId}/economy",                 economy_get),
        # ── Reaction Roles ──
        ("GET",    "/api/guilds/{guildId}/reactionroles",           reactionroles_get),
        ("DELETE", "/api/guilds/{guildId}/reactionroles",           reactionroles_delete),
        # ── Birthday ──
        ("GET",    "/api/guilds/{guildId}/birthday",                birthday_get),
        ("PATCH",  "/api/guilds/{guildId}/birthday",                birthday_patch),
        # ── Podcast ──
        ("GET",    "/api/guilds/{guildId}/podcast",                 podcast_get),
        ("PATCH",  "/api/guilds/{guildId}/podcast",                 podcast_patch),
        # ── Announcements ──
        ("GET",    "/api/guilds/{guildId}/announcements",           announcements_get),
    ]

    # Group by path (aiohttp_cors requirement — each resource added once)
